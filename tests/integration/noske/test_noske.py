...
class TestNoskeUI(unittest.TestCase):
    ...
    def test_is_up(self):
        ...
        self.assertIsNotNone(response, 'NoSketch Engine did not start correctly.')
        self.assertTrue(response.ok, 'NoSketch Engine did not start correctly.')
    ...
    def do_query(self, query: str, language: Optional[str] = None) -> int:
        ...
        result = self.page.locator('span.size[data-tooltip="Number of hits"]').text_content()
